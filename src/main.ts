...
import { addAudioToDom, moveUser, userState, calcDistance } from './utils'
import AgoraRTM, { RtmTextMessage } from 'agora-rtm-sdk';
import { position } from './utils';

const startBtn = (<HTMLInputElement>document.getElementById('start'));
const appidInput = (<HTMLInputElement>document.getElementById('appid'));
const channelInput = (<HTMLInputElement>document.getElementById('channel'));
const rtcTokenInput = (<HTMLInputElement>document.getElementById('rtctoken'));
const rtmTokenInput = (<HTMLInputElement>document.getElementById('rtmtoken'));
const uidInput = (<HTMLInputElement>document.getElementById('uid'));
...
const startApp = () => {
  ...
  const rtcToken = rtcTokenInput.value;
  const rtmToken = rtmTokenInput.value;
  ...
  let startCall = async () => {
    ...
    const sdkUid = await client.join(appId, channelId, rtcToken ? rtcToken : null, uid ? uid : null);
    await client.publish([localAudioTrack]).catch(e => { console.log(e) });
    console.log('joined RTC: ' + sdkUid);
    ...
    //@ts-ignore
    await rtmClient.login({ uid: String(sdkUid), token: rtmToken ? rtmToken : null })
    await rtmChannel.join()
    console.log('joined RTM: ' + sdkUid + ' ' + rtmChannel.channelId);
...
window.onload = () => {
  if (appidInput.value) {
    console.log('!hi')
    startBtn.disabled = false;
  } else console.log('!hi2', String(appidInput.value))
}